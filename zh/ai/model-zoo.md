...
**Model Zoo 关键目录速览：**
- **`model_zoo/cv`**：计算机视觉 SDK + `examples/`（检测/分类/分割/跟踪/人脸/姿态等）
...
- **vision**：`model_zoo/cv/include/vision_service.h`
...
bash components/model_zoo/cv/scripts/download_all_models.sh
```
...
bash components/model_zoo/cv/scripts/download_assets.sh
```
...
以下为 `components/model_zoo/cv/examples/` 下**全部示例**的运行命令（C++，执行过 `m` 后可直接运行）：

```bash
# 人脸识别（相似度）
arcface components/model_zoo/cv/examples/arcface/config/arcface.yaml

# 人脸检测
yolov5-face components/model_zoo/cv/examples/yolov5-face/config/yolov5-face.yaml

# 手势检测
yolov5_gesture components/model_zoo/cv/examples/yolov5_gesture/config/yolov5_gesture.yaml

# 目标检测
yolov8 components/model_zoo/cv/examples/yolov8/config/yolov8.yaml
yolov11 components/model_zoo/cv/examples/yolov11/config/yolov11.yaml

# 姿态估计
yolov8_pose components/model_zoo/cv/examples/yolov8_pose/config/yolov8_pose.yaml

# 实例分割
yolov8_seg components/model_zoo/cv/examples/yolov8_seg/config/yolov8_seg.yaml

# 图像分类 / 情绪识别
resnet components/model_zoo/cv/examples/resnet/config/resnet50.yaml
emotion components/model_zoo/cv/examples/emotion/config/emotion.yaml

# 多目标跟踪（视频/摄像头）
bytetrack components/model_zoo/cv/examples/bytetrack/config/bytetrack.yaml
ocsort components/model_zoo/cv/examples/ocsort/config/ocsort.yaml
```

每个示例的可选参数（如 `--image`、`--video`、`--use-camera`、`--output`、阈值等）请参考对应目录的 README（`components/model_zoo/cv/examples/*/README.md`）。
...
- **vision**：`vision_service.h`（详见 [cv/README.md](https://github.com/spacemit-robotics/model-zoo-vision/blob/main/README.md) 的「应用开发」章节）

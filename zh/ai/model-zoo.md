...
每个示例的可选参数（如 `--image`、`--video`、`--use-camera`、`--output`、阈值等）请参考对应目录的 README（`components/model_zoo/vision/examples/*/README.md`），或参阅 [model-zoo-vision README](https://github.com/spacemit-robotics/model-zoo-vision/blob/main/README.md)。
...
voice_chat --llm-url http://localhost:8080 --mcp-config ./components/agent_tools/mcp/examples/config_registry.json
```
...
> 说明：以下数据是基于 K1 / K3 平台（4线程推理，包含前后处理），按照组件中的 README 步骤实测。

K1：（数据截至 2026/3/10，持续迭代中，敬请关注）
...
> 说明：以下数据是基于 K3 平台（2 线程推理）按照组件 README 中步骤实测（数据截至 2026/3/10，持续迭代中，敬请关注）：
...
> 说明：以下数据是基于 K3 平台按照组件中的 README 步骤实测（数据截至 2026/3/10，持续迭代中，敬请关注）。
